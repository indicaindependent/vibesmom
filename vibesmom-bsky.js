...
var worker_default = {
  async fetch(request, env) {
    ...
    if (url.pathname === "/op-action" && request.method === "POST") {
      // Operator-gated: follow / repost / reply as vibesmom. Requires X-Op-Token === OPERATOR_OVERRIDE_TOKEN (in addition to X-Auth).
      const opTok = request.headers.get("X-Op-Token");
      if (opTok !== env.OPERATOR_OVERRIDE_TOKEN) {
        return new Response(JSON.stringify({ ok: false, error: "op token required" }), { status: 403, headers: { "Content-Type": "application/json" } });
      }
      let body;
      try { body = await request.json(); } catch (e) { return new Response(JSON.stringify({ ok:false, error:"bad json" }), { status:400, headers:{ "Content-Type":"application/json" } }); }
      const action = String(body.action || "");
      try {
        const sess = await bskyAuthFresh(env);
        const H = { "Content-Type": "application/json", "authorization": "Bearer " + sess.token };
        const results = {};
        // resolve a handle/did to a did
        async function toDid(actor) {
          if (!actor) return null;
          if (actor.startsWith("did:")) return actor;
          const rr = await fetch(`${BSKY_PUBLIC}/xrpc/app.bsky.actor.getProfile?actor=${encodeURIComponent(actor)}`);
          if (!rr.ok) return null;
          return (await rr.json()).did;
        }
        // get a post's cid from its uri
        async function getCid(uri) {
          const rr = await fetch(`${BSKY_PUBLIC}/xrpc/app.bsky.feed.getPostThread?uri=${encodeURIComponent(uri)}&depth=0`);
          if (!rr.ok) return null;
          return (await rr.json())?.thread?.post?.cid || null;
        }
        const nowIso = new Date().toISOString();
        if (action === "follow" || action === "all") {
          const did = await toDid(body.actor);
          if (did) {
            const r = await fetch(`${BSKY_PDS}/xrpc/com.atproto.repo.createRecord`, { method:"POST", headers:H,
              body: JSON.stringify({ repo: sess.did, collection:"app.bsky.graph.follow", record:{ "$type":"app.bsky.graph.follow", subject: did, createdAt: nowIso } }) });
            results.follow = r.ok ? "ok" : ("err " + r.status);
          } else results.follow = "actor not resolved";
        }
        if (action === "repost" || action === "all") {
          const uri = body.post_uri; const cid = body.post_cid || await getCid(uri);
          if (uri && cid) {
            const r = await fetch(`${BSKY_PDS}/xrpc/com.atproto.repo.createRecord`, { method:"POST", headers:H,
              body: JSON.stringify({ repo: sess.did, collection:"app.bsky.feed.repost", record:{ "$type":"app.bsky.feed.repost", subject:{ uri, cid }, createdAt: nowIso } }) });
            results.repost = r.ok ? "ok" : ("err " + r.status);
          } else results.repost = "missing uri/cid";
        }
        if (action === "reply" || action === "all") {
          const uri = body.post_uri; const cid = body.post_cid || await getCid(uri);
          const text = String(body.text || "").slice(0, 300);
          if (uri && cid && text) {
            const root = body.root_uri && body.root_cid ? { uri: body.root_uri, cid: body.root_cid } : { uri, cid };
            const r = await fetch(`${BSKY_PDS}/xrpc/com.atproto.repo.createRecord`, { method:"POST", headers:H,
              body: JSON.stringify({ repo: sess.did, collection:"app.bsky.feed.post", record:{ "$type":"app.bsky.feed.post", text, reply:{ root, parent:{ uri, cid } }, createdAt: nowIso, langs:["en"] } }) });
            const j = r.ok ? await r.json() : null;
            results.reply = r.ok ? (j.uri || "ok") : ("err " + r.status);
          } else results.reply = "missing uri/cid/text";
        }
        return new Response(JSON.stringify({ ok: true, handle: sess.handle, results }), { headers: { "Content-Type": "application/json" } });
      } catch (e) {
